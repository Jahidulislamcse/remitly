...
{
    $configarray = array( "name" => "panelservice Licensing Manager", "description" => "License module for WHMCS.<br />more info <a href=\"https://panelservice.site\" target=\"_blank\">https://panelservice.site</a>", "version" => "1", "author" => "panelservice.site", "language" => "english", "fields" => array(  ) );
...
function licensing_output($vars)
{
    ...
    if( !$action ) 
    {
        ...
        $ch = curl_init();
        $headers = array();
        curl_setopt($ch, CURLOPT_URL, "https://rbnet-soft.my.id/resellerapi/getblanace?token=$token");
        // SSL important
        ...
        $ch = curl_init();
        $headers = array();
        curl_setopt($ch, CURLOPT_URL, "https://rbnet-soft.my.id/resellerapi/getstatus?token=$token");
        // SSL important
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        ...
        $ch = curl_init();
        $headers = array();
        curl_setopt($ch, CURLOPT_URL, "https://rbnet-soft.my.id/resellerapi/getmsg?token=$token");
        // SSL important
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        ...
        echo "$msg";
                                      ...
                                      echo "                    </div>\n                </div>\n            </div>\n     <br><br>       <div class=\"col-sm-6\">\n                <div class=\"panel panel-default panel-accent-green\">\n                    <div class=\"panel-heading\">\n                        <h3 class=\"panel-title\">Total license list</h3>\n                    </div>\n                    <div class=\"panel-body\">\n         <a class=\"btn btn-default btn-block\" onclick=\"location.href='https://rbnet-soft.my.id/resellerapi/getlist?token=$token'\">Click here to view licenses from API</a>       ";
        ...
        $ch = curl_init();
        $headers = array();
        curl_setopt($ch, CURLOPT_URL, "https://rbnet-soft.my.id/resellerapi/getpackage?token=$token");
        // SSL important
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
 ...
 echo"       <style>
       ...
       if ($level == "Bronze"){
        echo "<a class = \"button\"; style = \"color:#cd7f32\">Reseller level : Full</a>";
}
else {
      echo "Reseller level : <aa>$level</aa>";
        ...
        $products = WHMCS\Product\Product::where("servertype", "=", "gblicensingv1")->get();
        foreach( $products as $product ) 
        {
            echo "<option value=\"" . $product->id . "\">" . $product->name . "</option>";
        }
        $addons = WHMCS\Product\Addon::where("module", "=", "gblicensingv1")->get();
    ...
    else
    {
        if( $action == "list" ) 
        {
            ...
            $result = select_query("tblproducts", "id,name", array( "servertype" => "gblicensingv1" ), "name", "ASC");
            while( $data = mysql_fetch_array($result) ) 
            {
                echo "<option value=\"" . $data["id"] . "\">" . $data["name"] . "</option>";
            }
           
            $addons = WHMCS\Product\Addon::where("module", "=", "gblicensingv1")->orderBy("name")->get();
            foreach( $addons as $addon ) 
            {
                echo "<option value=\"a" . $addon->id . "\">" . $addon->name . "</option>";
            }
