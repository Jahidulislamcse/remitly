...
@php
    $pageTitle = isset($title) ? $title : 'Recharge Requests';
@endphp
...
@section('panel')
    ...
    <input type="text" id="searchInput" class="search-box" placeholder="Search recharge...">
        ...
        @forelse($lists as $list)
            <div class="topup-card" data-status="{{ $list->status }}">
                ...
                <div class="topup-slot">
                    ...
                    @if ($list->mobile)
                        <div>
                            <strong>Recepient:</strong> {{ $list->mobile }}
                        </div>
                    @endif

                    @if ($list->operator)
                        <div>
                            <strong>Operator:</strong> {{ $list->operator }}
                        </div>
                    @endif
                    ...
                    @if ($list->created_at)
                        <div>
                            <strong>Date:</strong> {{ $list->created_at->format('d M Y H:i') }}
                        </div>
                    @endif
                ...
                <div class="topup-slot">
                    ...
                    <div class="mt-2 topup-actions">
                        ...
                        @if ($list->status == 0)
                            <a class="btn btn-success btn-circle" href="{{ route('recharge.approve', $list->id) }}"
                                onclick="return confirm('Approve?')">
                                <i class="fa fa-check"></i>
                            </a>

                            <a class="btn btn-danger btn-circle" href="{{ route('recharge.reject', $list->id) }}"
                                onclick="return confirm('Reject?')">
                                <i class="fa fa-times"></i>
                            </a>
                        @endif

                        <a class="btn btn-dark btn-circle" href="{{ route('recharge.delete', $list->id) }}"
