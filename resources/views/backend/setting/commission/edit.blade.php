...
@section('meta')
    <title>{{ $title ?? 'Edit Commission' }} - {{ @siteInfo()->company_name }}</title>
@endsection

@section('panel')
<div class="container-fluid">
    <h4 class="mb-3">{{ $title ?? 'Edit Commission' }}</h4>

    <form action="{{ route('admin.commission.update', $commition->id) }}" method="POST" class="card card-body">
        @csrf
        @method('PUT')
